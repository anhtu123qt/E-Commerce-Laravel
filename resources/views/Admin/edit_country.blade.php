		...
		<div class="card card-body">
			...
			<form class="form-horizontal m-t-30" action="{{route('country.update',$country->id)}}" method="POST">
				...
				<div class="form-group">
					<label>Country <span class="help"></span></label>
					<input type="text" class="form-control" name="country" value="{{$country->country}}">
				</div>
				<div class="form-group">
					<label>Description</label>
					<textarea name="description" class="form-control" rows="5">{{$country->description}}</textarea>
				</div>
				<div class="form-group">
					<label>Image file upload</label>
					<input type="file" name="image" class="form-control">{{$country->image}}
				</div>
				<div class="card-footer">
					<button type="submit" class="btn btn-success" name="add">Update</button>
					<a href="{{route('country.index')}}"><button type="button" class="btn btn-success">Cancel</button></a>
				</div>
			</form>
